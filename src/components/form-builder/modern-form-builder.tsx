...
import { FormField, FormPage, FieldTemplate } from './types';
...
export function ModernFormBuilder({ formId: _formId, initialFields = [], onSave }: FormBuilderProps) {
  ...
  // Field management
  const addField = useCallback((type: FormField['type'], template?: FieldTemplate) => {
  ...
  }, [formName, fields, pages, isMultiStep]);
  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen bg-background relative">
        ...
        {/* Analytics Panel */}
        <AnalyticsPanel fields={fields} showAnalytics={showAnalytics} />
        ...
        {/* Main Content */}
        <div className={`flex flex-col lg:flex-row ${showAnalytics || showLogicRules ? 'h-[calc(100vh-280px)]' : 'h-[calc(100vh-125px)]'}`}>
          {/* Sidebar */}
          <Sidebar
